    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            TargetError::NoStatusSocket(iface) => {
                write!(f, "no status socket for interface {} (status_socket not configured?)", iface)
            }
...
fn discover_instances() -> Vec<Instance> {
    ...
    if let Ok(entries) = fs::read_dir(PID_DIR) {
        for entry in entries.flatten() {
            ...
            let Some(iface) = iface_from_pidfile(name_str) else { continue };
            let path = entry.path();
    ...
    if let Ok(entries) = fs::read_dir(RUN_DIR) {
        for entry in entries.flatten() {
            let name = entry.file_name();
            let Some(name_str) = name.to_str() else { continue };
            let Some(iface) = iface_from_sockname(name_str) else { continue };
            let inst = by_iface.entry(iface.to_string()).or_default();
    ...
    for hit in scan_proc_for_pesigitgd() {
        match hit.interface.as_deref() {
            ...
            None => {
                ...
                let matched = by_iface
                    .values_mut()
                    .find(|i| i.pid == Some(hit.pid));
...
fn scan_proc_for_pesigitgd() -> Vec<ProcHit> {
    ...
    let Ok(entries) = fs::read_dir("/proc") else { return hits };
    ...
    for entry in entries.flatten() {
        ...
        let Ok(cmdline_bytes) = fs::read(format!("/proc/{}/cmdline", pid)) else { continue };
        ...
        hits.push(ProcHit { pid, interface, cmdline });
    }
...
fn cmd_list(_args: &[String]) -> Result<(), TargetError> {
    let instances = discover_instances();
    let (named, unnamed): (Vec<_>, Vec<_>) = instances
        .into_iter()
        .partition(|i| i.interface.is_some());
    ...
    if !named.is_empty() {
        ...
        for inst in &named {
            ...
            if inst.pidfile.is_some() { sources.push("pidfile"); }
            if inst.status_socket.is_some() { sources.push("socket"); }
            if inst.cmdline.is_some() { sources.push("proc"); }
...
mod tests {
    ...
    fn iface_from_pidfile_accepts_typical() {
        ...
        assert_eq!(iface_from_pidfile("pesigitgd-enp2s0f0.pid"), Some("enp2s0f0"));
    }
    ...
    fn iface_from_sockname_accepts_typical() {
        ...
        assert_eq!(iface_from_sockname("status-enp2s0f0.sock"), Some("enp2s0f0"));
    }
    ...
    fn target_trims_whitespace() {
        assert_eq!("  eth0  ".parse::<Target>().unwrap(), Target::Interface("eth0".to_string()));
