...
//! QUIC-LB Connection ID extraction and decryption primitives.
//!
...
//!
//! The daemon layers `lookup_config(quic, &ConfigTable)` on top of these
//! primitives — that resolver lives in the daemon because it indexes a
//! daemon-owned slot table that also carries retry-service state.

use aes::Aes128;
use aes::cipher::{BlockDecrypt, BlockEncrypt, generic_array::GenericArray};

use crate::route::{Encryption, RouteConfig};

/// Extract the first CID octet from a QUIC packet (long or short header).
///
/// The top three bits of this octet are the QUIC-LB *config_id*. Returns
/// `None` if the buffer is too short or a long header carries an empty
/// DCID.
pub fn first_cid_octet(quic: &[u8]) -> Option<u8> {
    if quic.is_empty() {
        return None;
    }
    if quic[0] & 0x80 != 0 {
...
/// Extract raw DCID bytes from a QUIC packet without validating the
/// config rotation ID. Used for connection table lookups on unroutable
/// packets and as the building block for higher-level lookups.
